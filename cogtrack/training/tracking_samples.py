...
def _draw_reference(image: np.ndarray, bbox_xywh: TypingSequence[float]) -> np.ndarray:
    """在 RGB 副本上绘制绿色初始化框，不修改原图。"""
...
def _build_mosaic(
    ...
    *,
    ...
    """把带 GT 框的可信历史帧按时间顺序拼接为单张 RGB 图片。"""

    rendered: list[np.ndarray] = []
    for frame_id, image, bbox in panels:
        panel = _draw_reference(image, bbox)
        ...
        header = np.full((30, resized_width, 3), 245, dtype=np.uint8)
        cv2.putText(
            header,
            f"trusted frame {frame_id}",
            (6, 20),
        ...
        rendered.append(np.vstack((header, panel)))

    if not rendered:
        raise ValueError("构造 mosaic 至少需要一个有效历史帧")
    separator = np.full((panel_height + 30, 5, 3), 220, dtype=np.uint8)
...
def _candidate_frame_ids(
    ...
    *,
    ...
    if planned_frame_ids is not None:
        source_ids = sorted(set(int(frame_id) for frame_id in planned_frame_ids))
...
def _history_panels(
    ...
    *,
    ...
    panels: list[tuple[int, np.ndarray, tuple[float, float, float, float]]] = []
    for frame_id in range(current_frame_id - 1, anchor_frame_id, -1):
        ...
        if raw_bbox is None:
            continue
        image = _read_rgb(sequence.frames[frame_id])
        bbox = _clip_bbox_to_image(raw_bbox, image)
        if bbox is None:
            continue
...
def _record(
    *,
    ...
    return {
        ...
        "id": (
            f"{sequence.dataset}::{sequence.name}::{frame_id:08d}::"
        ...
        with temporary_handle as handle:
            for sequence in sequences:
                ...
                for frame_id in _candidate_frame_ids(
                    ...
                    current_image = _read_rgb(sequence.frames[frame_id])
                    presence = _presence(sequence, frame_id)
                    assert presence in {"present", "absent"}
                    bbox_norm: tuple[float, float, float, float] | None = None
                    if presence == "present":
                        current_bbox = _clip_bbox_to_image(_raw_bbox(sequence, frame_id), current_image)
                    ...
                    contexts: list[tuple[str, str, str | None, list[int], PromptSpec]] = []
                        ...
                        contexts.append(
                            (
                                ...
                                build_pair_prompt(
                                    ...
                                    reference_bbox_norm1000_xyxy=anchor_bbox_norm,
                    ...
                    if options.mode in {"mosaic", "both"}:
                        ...
                        if panels:
                            history_asset = asset_dir / f"history_before_{frame_id:08d}.jpg"
                            _write_rgb(
                                history_asset,
                                _resize_long_side(
                                    _build_mosaic(panels, panel_height=options.mosaic_panel_height),
                                    options.max_image_side,
                                ),
                                jpeg_quality=options.jpeg_quality,
                                overwrite=overwrite,
                            ...
                            history_frame_ids = [item[0] for item in panels]
                            contexts.append(
                                (
                                    "mosaic",
                                    "mosaic",
                    ...
                    for requested_mode, effective_mode, history_path, history_ids, prompt in contexts:
                        row = _record(
                            sequence=sequence,
                            frame_id=frame_id,
