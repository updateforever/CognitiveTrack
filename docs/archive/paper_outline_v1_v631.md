# CognitiveTrack 论文大纲 v1（历史 v6.3.1 草案）
...
Input: Video frames + initial bbox + initial identity description
       ↓
[VLT-v6.3.1 Core SFT: learn presence + bbox]
       ↓
...
**Data sampling**:
- Present:absent = 80:20 at unique-case level
...
1. **完成 Core SFT 训练与 Base/Core 对比**
   - [x] 生成全量 VLT-v6.3.1 Core 数据（50,220 cases / 57,426 views）
...
| 周次 | 任务 | 交付物 |
|-----|------|--------|
| Week 1-2 (Aug 14-27) | Core 数据已完成；Core LoRA smoke、正式训练和 Tiny 评测 | Base/Core 对比数据 |
...
**下一步**：先完成 Qwen3-VL-4B Core LoRA smoke、正式训练和同协议 Tiny 对照；文献与
baseline 调研并行推进，但不阻塞 Core 实验。
