...
GT presence/bbox 数据
        ↓
VLT-v6.3 core SFT（存在性 + 定位）
        ↓
region-caption/ref 教师生成目标状态事件银标
        ↓
memory SFT（何时更新 + 更新后的完整状态）
        ↓
...
有效历史证据；它们采用旧输入/输出协议，不能替代 VLT-v6.3 的 core SFT、状态标签、
memory SFT、TU-GRPO 或 Full 评测。真实进度见
...
- 文本 1：不可变的 `initial_identity_description`；
- 文本 2：可替换的 `current_target_state`，初始值等于身份描述；
- 输出：`target_status`、当前 bbox、可空的 `memory_update`。

`memory_update=null` 表示沿用当前状态。非空值不是只写“变成背面”一类增量，而是一个
短、可独立理解、保留身份线索的完整替换状态；这样重放任意帧时不需要拼接一串可能
矛盾的历史文本。目标消失由 `target_status=absent` 表达，绝不清空或改写记忆。

训练模型使用 `prompt_version=6.3.0` 的 native 极简 System Prompt：只定义不可覆盖的
...
使用 LaSOT/TNL2K/MGIT train 的 GT presence 和 bbox。present/absent 初始保持约 7:3，
reference/history 均严格早于 current。三字段结构全部保留，但仅屏蔽
`memory_update` 的值，不把普通跟踪样本伪监督为“永不更新”。

### 3.2 Memory SFT：冷启动状态维护

只对事件候选和 hard-null 样本调用离线教师。教师可以看带 GT 框的全图、目标区域辅助
视图以及仅用于验标的后续支持帧；学生训练输入仍严格使用在线可得的三图上下文。GT
负责 bbox/presence，caption/ref 模型只负责提出和校验文本状态，不能替代定位真值。

身份与状态分成两层：

- `initial_identity_description`：来自首帧和安全的初始化文本，永久不变；
...
同时惩罚频繁更新、身份矛盾、冗长文本和 absent 更新。完整定义见
[`grpo.md`](grpo.md)。
...
主表固定同一 VLT-v6.3 推理协议：

| 模型 | Core SFT | Memory SFT | TU-GRPO | 目的 |
| --- | :---: | :---: | :---: | --- |
| Qwen3-VL-4B Base |  |  |  | 零样本下限 |
| Core | ✓ |  |  | 跟踪监督收益 |
| Core + Memory | ✓ | ✓ |  | 状态银标收益 |
| Core + Memory + TU-GRPO | ✓ | ✓ | ✓ | 完整方法 |

必要消融：

- initial identity only / 加 current state；
- generic caption / region caption / region caption + verifier；
- 所有帧 caption / 事件候选采样；
- 非空增量 / 完整替换状态；
