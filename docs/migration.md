...
- `qwen_abs_pixel`：模型 JSON 字段名为 `bbox_pixel_xyxy`，用于 Qwen2.5-VL；
- `norm1000`：字段名为 `bbox_norm1000_xyxy`，用于 Qwen3-VL（以及旧 Qwen2-VL）。

字段名随协议改变是刻意的：沿用 `bbox_norm1000_xyxy` 这个名字会同时把模型和
读代码的人往归一化方向带。协议名和模型空间尺寸都逐帧写入
